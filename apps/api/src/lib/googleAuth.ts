...
export async function resolveGoogleUser(prisma: PrismaClient, profile: GoogleProfile): Promise<User> {
  ...
  if (invite) {
    return prisma.$transaction(async (tx) => {
      const user = await tx.user.create({
        data: { email: profile.email, googleId: profile.googleId, tenantId: invite.tenantId },
      });
  ...
  return prisma.$transaction(async (tx) => {
    const tenant = await tx.tenant.create({ data: { name: tenantNameFromEmail(profile.email) } });
    return tx.user.create({
      data: { email: profile.email, googleId: profile.googleId, tenantId: tenant.id },
    });
  });
}
